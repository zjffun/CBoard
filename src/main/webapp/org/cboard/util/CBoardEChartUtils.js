...
var updateEchartOptions = function(tuningOpt, rawOpt) {
  if (tuningOpt) {
    ...
    if (tuningOpt.legendShow == false) {
      ...
      rawOpt.legend === undefined
        ? (rawOpt.legend = angular.copy(echartsBasicOption.legend))
        : null;
      tuningOpt.legendX ? (rawOpt.legend.x = tuningOpt.legendX) : null;
      tuningOpt.legendY ? (rawOpt.legend.y = tuningOpt.legendY) : null;
      tuningOpt.legendOrient
        ? (rawOpt.legend.orient = tuningOpt.legendOrient)
        : null;

      rawOpt.legend.textStyle = rawOpt.legend.textStyle || {};
      if (tuningOpt.legendTextColor) {
        rawOpt.legend.textStyle.color = tuningOpt.legendTextColor;
      }
      if (tuningOpt.legendTextFontSize) {
        rawOpt.legend.textStyle.fontSize = tuningOpt.legendTextFontSize;
      }
    }
