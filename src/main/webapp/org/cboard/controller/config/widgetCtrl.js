 ...
 */
"use strict";
cBoard.controller("widgetCtrl", function(
  $scope,
  $state,
  $stateParams,
  $http,
  ...
  $http.get("dashboard/getDatasourceList.do").success(function(response) {
    ...
    getWidgetList(function() {
      if ($stateParams.id) {
        $scope.editWgt(
          _.find($scope.widgetList, function(w) {
  ...
  $scope.editExp = function(col) {
    ...
    $uibModal.open({
      ...
      controller: function($scope, $uibModalInstance) {
        ...
        $scope.expAceOpt = expEditorOptions($scope.selects, aggregate, function(
          _editor
        ) {
        ...
        $scope.ok = function() {
          if (!$scope.data.alias) {
            ModalUtils.alert(
              translate("CONFIG.WIDGET.ALIAS") + translate("COMMON.NOT_EMPTY"),
              "modal-warning",
              "lg"
            );
            return;
  ...
  var addHelpMessage = function() {
    var rowKey =
      "HELP_MESSAGE." +
      $scope.curWidget.config.chart_type.toUpperCase() +
      ".ROW";
    var columnKey =
      "HELP_MESSAGE." +
      $scope.curWidget.config.chart_type.toUpperCase() +
    ...
    var column =
      translate(columnKey) == columnKey ? null : translate(columnKey);
    var filter =
      translate(filterKey) == filterKey ? null : translate(filterKey);
  ...
  var validation = function() {
    ...
    if (!$scope.widgetName) {
      $scope.alerts = [
        {
          msg:
            translate("CONFIG.WIDGET.WIDGET_NAME") +
    ...
    if ($scope.customDs == false && $scope.curWidget.datasetId == undefined) {
      $scope.alerts = [
        {
          msg:
            translate("CONFIG.WIDGET.DATASET") + translate("COMMON.NOT_EMPTY"),
          type: "danger"
        }
      ];
      return false;
    ...
    if ($scope.customDs == true) {
      for (var i = 0; i < $scope.params.length; i++) {
        ...
        if (
          ...
          if (msg && msg.length > 0) msg = translate(msg[0]);
          else msg = label;
          $scope.alerts = [
            {
  ...
  var changeChartStatus = function() {
    for (var type in $scope.chart_types_status) {
      ...
      if (
        _.size(config.keys) == 0 &&
        _.size(config.groups) == 0 &&
    ...
    dataService.viewQuery(
      ...
      function(query) {
        var querybr = query
          ...
          .replace(/\t/g, "&nbsp;&nbsp;&nbsp;&nbsp;");
        $("#viewQuery_widget").html(
          "<div class='alert alert-info' role='alert' style='text-align: left;'><p style='color: black'>" +
            querybr +
            "</p></div>"
        );
  ...
  $scope.preview = function() {
    ...
    $("#preview_widget").html(
      "<div id='preview' style='min-height: 450px; user-select: text;'></div>"
    );
    // --- end ---
  ...
  $scope.saveWgt = function() {
    ...
    o.name = $scope.widgetName
      .slice($scope.widgetName.lastIndexOf("/") + 1)
      .trim();
    o.categoryName = $scope.widgetName
      .substring(0, $scope.widgetName.lastIndexOf("/"))
      .trim();
    if (o.categoryName == "") {
    ...
    if ($scope.optFlag == "new") {
      $http
        .post("dashboard/saveNewWidget.do", { json: angular.toJson(o) })
      ...
      o.id = $scope.widgetId;
      $http
        .post(updateUrl, { json: angular.toJson(o) })
        .success(function(serviceStatus) {
          if (serviceStatus.status == "1") {
            getWidgetList();
            getCategoryList();
  ...
  $scope.editWgt = function(widget) {
    $http
      .post("dashboard/checkWidget.do", { id: widget.id })
  ...
  var doEditWgt = function(widget) {
    ...
    loadDataset(function() {
      ...
      loadDsFilterGroups();
      buildSchema();
      dataService.linkDataset(
        $scope.curWidget.datasetId,
        $scope.curWidget.config
      );
    });
    addWatch();
  ...
  $scope.deleteWgt = function(widget) {
    ModalUtils.confirm(
      ...
      function() {
        $http
          .post("dashboard/deleteWidget.do", { id: widget.id })
          .success(function(serviceStatus) {
            if (serviceStatus.status == "1") {
              getWidgetList();
            } else {
  ...
  $scope.doConfigParams = function() {
    $http
      .get("dashboard/getConfigParams.do", {
        params: {
